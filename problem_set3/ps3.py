...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.tiles = {(w,h):int(self.dirt_amount) for w in range(math.floor(self.width)) for h in range(math.floor(self.height))}
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if not self.is_position_in_room(pos):
            raise KeyError('Position is not valid inside Room')
        pos_tuple = (math.floor(pos.get_x()), math.floor(pos.get_y()))
        dirt = self.tiles[pos_tuple]
        if capacity > dirt:
            self.tiles[pos_tuple] = 0
        else:
            self.tiles[pos_tuple] -= capacity
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        return sum([1 for _ in self.tiles.values() if _ == 0])
...
class EmptyRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        Returns: an integer; the total number of tiles in the room
        """
        return self.get_room_width() * self.get_room_height()
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        pos,dir = self.get_robot_position(), self.get_robot_direction()
        new_pos = Position.get_new_position(pos,dir,self.speed)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trials in range(num_trials):
        ...
        time_to_clean.append(time_steps)
    return sum(time_to_clean)/len(time_to_clean)
...
#       of a 20x20 room?
#    - As the number of robots grows, the performance delta of the two robots shrinks
#    - But when the number of robots is small, the number of steps taken to clean is quite wide

# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions 
#       10x30, 20x15, 25x12, and 50x6?
#    - As rooms become less square and more rectangular, faulty robots take
#       significantly longer to clean 80% of a room
